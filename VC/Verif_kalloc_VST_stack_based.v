...
Require Import VC.Verif_kalloc_Fun_Spec.

Fixpoint listrep (il: list Z) p : mpred :=
 match il with
 | i::il' => EX y, 
        malloc_token Ews (Tstruct _run noattr) p *
        data_at Ews (Tstruct _run noattr) (y) p *
	listrep il' y
 | nil => !! (p = nullval)
 end.

Definition stack (il: list Z) p :=
 EX q kk,
  malloc_token Ews (Tstruct _kmem noattr) p * 
  data_at Ews (Tstruct _kmem noattr) q p *
  listrep il kk.

Arguments stack il p : simpl never.

Definition kinit_spec : ident * funspec :=
 DECLARE _kinit
 WITH gv: globals
 PRE [ ] 
    PROP () PARAMS() GLOBALS(gv) SEP (mem_mgr gv)
 POST [ tptr (Tstruct _kmem noattr) ] 
    EX p, PROP ( ) RETURN (p) SEP (stack nil p; mem_mgr gv).
...
Proof.
  ...
  - unfold listrep. destruct p; Intro y; entailer!.
    split; intros; auto; inversion H2.
Qed.
...
Proof.
  intros. destruct il; unfold stack; Intros q; entailer!. 
Qed.
...
start_function.
  unfold stack.
  Intros q.
  forward.  unfold  listrep.
  Intros q_next.
  forward. forward. forward.
  forward_call (Tstruct _cons noattr, q, gv).
  - simpl.
    destruct (EqDec_val q nullval).
    + entailer!.
    + entailer!.
  - forward. unfold stack. Exists q_next. entailer!. unfold listrep. entailer!.
Qed.
(** [] *)
...
- destruct (eq_dec q nullval); try contradiction; try entailer.
- destruct (eq_dec q nullval); try contradiction; try entailer. forward_call (1). entailer!.
- forward. entailer!.
- destruct (eq_dec q nullval).
  + Intros. subst. destruct H0. auto.
  + Intros. forward. simpl. forward. forward. forward. entailer!. unfold stack. 
    Exists q. entailer!. unfold listrep at 2. Exists q0. entailer!. unfold listrep. entailer.
Qed.
(** [] *)
...
start_function.
(* FILL IN HERE *)
forward_call (Tstruct _stack noattr, gv). (* malloc *)
Intros p.
forward_if (
  PROP (p <> nullval )
  LOCAL (temp _p p)
  ...
  malloc_token Ews (Tstruct _stack noattr) p * data_at_ Ews (Tstruct _stack noattr) p; mem_mgr gv)).

- destruct (eq_dec p nullval); entailer!.
- destruct (eq_dec p nullval); forward_call (1); entailer!.
- destruct (eq_dec p nullval); forward; entailer!.
- destruct (eq_dec p nullval).
  + subst. Intros; try contradiction.
  + Intros. forward. forward. unfold stack. Exists p. Search (_ = _). 
  assert ((p = p) = True). {
  ...
  Exists nullval.
  entailer!. 
  assert (listrep [] nullval = emp). {
    unfold listrep. 
    replace (nullval = nullval) with True. 
    Search ( !! _ && _). apply prop_true_andp'; auto.
