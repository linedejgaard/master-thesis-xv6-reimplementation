...
Lemma inc_spec (l : val) (xs : list Z) :
    (* `isList l ((λ x : Z, #x) <$> xs)` asserts that `l` is a linked list representation of the list `xs` where each element is converted to a value using the lambda function `(λ x : Z, #x)`. *)
    (* `((λ x : Z, #x) <$> xs)` is shorthand for `map (λ x : Z, #x) xs`, which means applying the function `(λ x : Z, #x)` to each element of `xs`, converting each element to a value. *)
  {{{ isList l ((λ x : Z, #x) <$> xs) }}}
    inc l
...
Proof.
  ...
  - iIntros (l acc ys) "%Φ [-> H2] HΦ".
    wp_rec. wp_pures. iApply "HΦ". done.
  -  iIntros (l acc ys Φ) "[(%hd & %l' & -> & Hhd & Hl) Hacc] HΦ". (* intros ys l1 l2, and also phi, and -> makes subst *)

    wp_rec. (* kind of unfolds it *)
    wp_pures. (* kind of simplifies all the logic things *) 

    wp_load. (* used when ! *) 
    wp_proj. (* fixes Fst *) 
    wp_let. (* "executes" the let, such that x is set to x *)

    wp_load. (* used when !  *) 
    wp_proj. (* fixes Snd *) 
    wp_let. (* "executes" the let, such that l' is set to l' *)
    wp_store. (*  used when <- *)

    (* with "[Hl Hhd Hacc]":  This part specifies which hypotheses from the current proof context should be used to satisfy the premises of IH. *)
    wp_apply (IH l' (InjRV #hd) (x :: ys) with "[Hl Hhd Hacc]"). {
    (* IH says forall... I just give it l, acc, ys and not phi. and write which hypothesis to take with me *)
...
Proof.
  ...
  wp_rec.
  wp_pures.
  wp_apply (reverse_append_spec _ _ _ [] with "[$H1]"). {
    done.
  }
...
Proof.
  ...
  all: simpl.
  (* exercise *) 
  - iIntros (a l) "%Φ (-> & emp & I & #Hf) HΦ".
    (* -> is just use, and substitute, everything before the -∗ (magic wand) operator in Iris is considered to be in the same group, and that is the reason why parenthesis are used*)
    wp_rec. 
    wp_pures.
    iModIntro.
    iApply "HΦ".
    iSplitR "I"; try done.
  - iIntros (a l) "%Φ ((%hd & %l' & -> & Hhd & Htl) & [P0 Ps] & I & #Hf) HΦ".
    wp_rec.
    wp_pures.
    wp_load. wp_proj. wp_let. wp_load. wp_proj. wp_let. 
    wp_apply (IHxs with "[$Htl $Ps $I $Hf]").
    iIntros (r) "[Hl I]".
    wp_apply ("Hf" with "[$P0 $I]"). (* use $ --> use directly, not sure about [] *)
    iIntros (r') "I".
    iApply "HΦ". (* I think only wp_apply is for when there is a wp *)
    iFrame.
    done.
Qed.
