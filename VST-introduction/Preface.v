...
Require VC.sum.  (* If this line fails, do 'make kalloc.vo' *)
Goal VC.sum.Info.bitsize = VST.veric.version.bitsize.
reflexivity ||
let b1 := constr:(VST.veric.version.bitsize) in let b1 := eval compute in b1 in
let b2 := constr:(VC.sum.Info.bitsize) in let b2 := eval compute in b2 in
