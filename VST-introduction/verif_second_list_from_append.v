...
+ Intros u. remember (v::s1') as s1. forward. forward. Exists x. entailer!.
   simpl. unfold listrep; fold listrep. Exists y. entailer!.
   destruct s1'.
   * Search ([] ++ _). rewrite app_nil_l. unfold listrep. entailer!.
   * 
   simpl. unfold listrep; fold listrep. entailer. 
   Exists 
   
   nået hertil!!! er u og y samme pointer???
...
Fixpoint lseg (sh: share)
 ...
 | h::hs => !! (x<>z) && 
              EX y:val,
                data_at sh t_node (h,y) x * lseg sh hs y z
...
Lemma lseg_cons: forall sh (v u x: val) s,
   readable_share sh ->
 data_at sh t_node (v, u) x * lseg sh s u nullval
...
Lemma lseg_cons': forall sh (v u x a b: val) ,
   readable_share sh ->
 data_at sh t_node (v, u) x * data_at sh t_node (a,b) u
 |-- lseg sh [v] x u * data_at sh t_node (a,b) u.
Proof.
intros.
...
Lemma lseg_app': forall sh s1 s2 (a w x y z: val),
   readable_share sh ->
   lseg sh s1 w x * lseg sh s2 x y * data_at sh t_node (a,z) y |--
   lseg sh (s1++s2) w y * data_at sh t_node (a,z) y.
Proof.
 intros.
