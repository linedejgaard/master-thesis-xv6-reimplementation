...
(* free and alloc *)
Definition free_freelistrep (sh: share) (new_head original_head: val) (n: nat) : mpred :=
  !! malloc_compatible (sizeof t_run) new_head &&
  data_at sh t_run original_head new_head * freelistrep sh n original_head.

Definition alloc_freelistrep (sh: share) (original_head: val) (n: nat) : mpred :=
EX next: val,
    !! malloc_compatible (sizeof t_run) original_head &&
    data_at sh t_run next original_head *
    freelistrep sh (n-1) next.
  
(* If we have free_freelistrep, then the size of the list increases by 1, and the new head becomes the new head of the freelist *)
Lemma free_freelist_correct : (* if we free a new_head, this head is added and the list is expanded *)
    forall sh n original_head new_head,
      free_freelistrep sh new_head original_head n |-- freelistrep sh (S n) new_head.
Proof.
    intros.
    unfold free_freelistrep.
    unfold freelistrep; fold freelistrep. Exists original_head. entailer!.
Qed.

(* If we have freelistrep sh n p with 0 < n, then it is possible to do alloc_freelistrep sh p n *)
Lemma alloc_freelistrep_non_empty_correct :
  forall sh n p,
    (0 < n)%nat ->
    freelistrep sh n p |-- alloc_freelistrep sh p n.
Proof.
  intros.
  unfold alloc_freelistrep.
  destruct n; simpl.
  - unfold freelistrep. lia. (* Case n = 0 is impossible due to n > 0 *)
  - replace (n - 0)%nat with n; try rep_lia.
  unfold freelistrep at 1; fold freelistrep. Intros next; Exists next. entailer.
Qed.
(* free and alloc end *)
...
DECLARE _kfree1
    ...
    POST [ tvoid ]
        ...
        RETURN () 
        SEP (
            free_freelistrep sh new_head original_freelist_pointer n *
...
DECLARE _client1
WITH sh : share, new_head:val, original_freelist_pointer:val, xx:Z, gv:globals, n : nat, number_structs_available:nat
PRE [ tptr tvoid ]
    PROP(
        writable_share sh /\
...
- (*unfold abbreviate in POSTCONDITION.*) forward_call (sh, new_head, xx, (S n), gv). (* call kfree1 *)
    ...
    + destruct (eq_dec new_head nullval) eqn:enh.
        ...
        * forward. Exists next. entailer!. assert ((S n - 1)%nat = n); try rep_lia. rewrite H10. entailer!.
Qed.
