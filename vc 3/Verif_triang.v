...
induction j.
- simpl; auto.
- apply H0; try lia. rewrite Nat2Z.inj_succ. replace (Z.succ (Z.of_nat j) - 1) with (Z.of_nat j); try lia.
  apply IHj; try lia.
Qed.
(** [] *)
...
(* FILL IN HERE *)
destruct (Z_gt_dec i 0); try lia. simpl. 
Search (_ * (_ + _)).
rewrite Z.mul_add_distr_l. rewrite H1. ring_simplify. auto.
  - auto.
Qed.
...
(* FILL IN HERE *)  
  ...
  - symmetry. apply add_list_decreasing_eq_alt. auto. 
Qed.
(** [] *)
...
Proof.
(* FILL IN HERE *)
  intros. induction il; simpl; intros.
  - unfold stack. Intros q. unfold listrep. entailer!.
  - unfold stack. Intros q. unfold listrep; fold listrep. entailer!.
Qed. 
...
Proof.
  ...
  forward_while (
    EX i : Z,
    PROP ( 0 <= i <= n )
    LOCAL (temp _i (Vint (Int.repr i)); gvars gv; temp _st st; temp _n (Vint (Int.repr n)))
    SEP (stack (decreasing i) st; mem_mgr gv)
  ).
  - entailer. EExists. entailer!; try rep_lia. 
  - entailer!.
  - forward. forward_call. EExists. entailer!; try rep_lia.
    replace (decreasing (i + 1)) with (if Z_gt_dec (i + 1) 0 then (i+1) :: decreasing ((i + 1) - 1) else []). 
    2: {
      destruct (Z_gt_dec (i + 1) 0).
      - symmetry. rewrite decreasing_equation.  destruct (Z_gt_dec (i + 1) 0); try contradiction.
        auto.
      - exfalso. apply n0. lia.
    }
    destruct (Z_gt_dec (i + 1) 0).
    + unfold stack. entailer!. EExists. entailer!.
      unfold listrep. Intros y. Exists y. entailer!. fold listrep. 
      replace (i + 1 - 1) with i. entailer!. lia.
    + exfalso. apply n0. lia.

  - forward. entailer!. unfold stack. Intros q. replace n with i.
    2: { try rep_lia.  } Exists q. entailer!.
Qed.
(** [] *)
...
Proof.
  (* FILL IN HERE *)
  intros. induction al.
  - simpl. Search (0 + _). rewrite Z.add_0_l. auto.
  - simpl. rewrite IHal. try rep_lia. 
Qed.
...
(* FILL IN HERE *) 
  intros.
  assert ((add_list il) = add_list (sublist 0 lo il) + add_list (sublist lo hi il) + add_list (sublist hi (Zlength il) il)). {
    rewrite <- add_list_app. rewrite <- add_list_app; try lia. rewrite <- sublist_split; try lia. rewrite <- sublist_split; try lia.
    rewrite sublist_same; auto.
  }
  rewrite H3 in H2.
  Search (_ <= _ <= _).
  split; destruct H2.
  - apply add_list_nonneg. 
    Check Z.le.
    apply Forall_sublist with (lo := lo) (hi := hi) in H1. auto.
  - Search (_ + _ <= _).
  apply list_nonneg_le_K with (a := add_list (sublist 0 lo il)) (c := add_list (sublist hi (Zlength il) il)); try rep_lia; try lia; try (apply add_list_nonneg).
    + apply Forall_sublist with (lo := 0) (hi := lo) in H1. auto.
    + apply Forall_sublist with (lo := lo) (hi := hi) in H1. auto.
    + apply Forall_sublist with (lo := hi) (hi := Zlength il) in H1. auto.
Qed.
(** [] *)
...
assert (0 <= add_list il). {
  (* FILL IN HERE *) 
  apply add_list_nonneg; auto. 
}
 assert (0 <= add_list (sublist 0 i il) <= Int.max_signed). {
  (* FILL IN HERE *) 
  apply add_list_sublist_bounds; try lia; auto.
 }
 ...
 assert (0 <= Znth i il <= Int.max_signed). {
  ...
  - apply add_list_sublist_bounds; try lia; auto.
   (* FILL IN HERE *)
  - (* FILL IN HERE *) 
    rewrite sublist_len_1. simpl. lia. auto.
}
...
+ (* Prove that the precondition implies the loop invariant *)
  EExists. entailer!. rewrite sublist_same; try lia; auto.
  (* FILL IN HERE *)
...
forward_call (st, Znth i il, sublist (i+1) (Zlength il) il, gv).
 (** This [forward_call] couldn't quite figure out the "Frame" for the 
  ...
  sublist i (Zlength il) il = Znth i il :: sublist (i+1) (Zlength il) il.
   ...
   then just do [cancel].  *)
   assert (sublist i (Zlength il) il = Znth i il :: sublist (i + 1) (Zlength il) il). {
    replace (Znth i il :: sublist (i + 1) (Zlength il) il) with ([Znth i il] ++ sublist (i + 1) (Zlength il) il). 
    * rewrite <- sublist_len_1. rewrite <- sublist_split; try lia; auto. split; auto. destruct H2; auto.
    * simpl; auto.
   }
  rewrite <- H3. entailer!.
...
forward.
forward.
forward_while (EX i:Z, EX s:Z,
   PROP(0 <= i <= Zlength il; s = add_list (sublist 0 i il) ) (* s is the accumulator *)
   LOCAL (temp _st st; 
          temp _i (Vint (Int.repr i)); 
   ...
   SEP (stack (sublist i (Zlength il) il) st; mem_mgr gv)).
+ (* Prove that the precondition implies the loop invariant *)
  EExists. EExists. entailer!. rewrite sublist_same; try lia; auto. 
  (* FILL IN HERE *)
+ (* "type-check the expression": prove the loop test evaluates *)
entailer!.
+ (* Prove the loop body preserves the loop invariant *)
forward_call (st, Znth i il, sublist (i+1) (Zlength il) il, gv).
 (** This [forward_call] couldn't quite figure out the "Frame" for the 
  ...
  sublist i (Zlength il) il = Znth i il :: sublist (i+1) (Zlength il) il.
   ...
   replace (Znth i il :: sublist (i + 1) (Zlength il) il) with (sublist i (Zlength il) il).
   entailer!.
  
  forward.
  - entailer!. apply add_another; auto. try rep_lia.
  - forward. Exists (i+1, s+Znth i il). entailer!. split; auto; try rep_lia.
   replace (add_list (sublist 0 i il) + Znth i il) with (add_list (sublist 0 i il) + add_list ([Znth i il])) by (simpl; try rep_lia).
   Search add_list.
   rewrite <- add_list_app. Search sublist.
   rewrite <- sublist_len_1; auto; try rep_lia.
   rewrite <- sublist_split; try rep_lia.
+ forward. assert (i = Zlength il). { try rep_lia. } subst. 
  unfold stack; entailer!.
  - f_equal. f_equal. rewrite sublist_same; try rep_lia.
  - Intros q. Exists q. entailer.
    Search sublist. rewrite sublist_nil. entailer!.
Qed.
(** [] *)
...
- entailer!.
- split.
  + do 11 rewrite decreasing_equation.
    destruct (Z_gt_dec).
    * simpl. Search Zlength. rewrite Zlength_correct. simpl. try rep_lia.
    * rewrite Zlength_correct. simpl. try rep_lia.
  + split; try rep_lia.
    * do 11 rewrite decreasing_equation.
      simpl. Search Forall. do 10 (rewrite Forall_cons_iff; split; try rep_lia).
      apply Forall_nil.
    * do 11 rewrite decreasing_equation. simpl. rep_lia.
- forward.
Qed.
(** [] *)
